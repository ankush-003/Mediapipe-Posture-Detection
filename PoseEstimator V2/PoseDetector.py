...
class PoseDetector():
    ...
    def findPose(self, img, draw=True, color=(0,255,0), thickness=2):
        ...
        if self.results.pose_landmarks:
            if draw:
                self.mpDraw.draw_landmarks(img, self.results.pose_landmarks, self.mpPose.POSE_CONNECTIONS, self.mpDraw.DrawingSpec(color=(0,0,255), thickness=thickness, circle_radius=4), self.mpDraw.DrawingSpec(color=color, thickness=thickness, circle_radius=2))
        return img
    
    def findPosition(self, img, draw=True, color=(0,0,255), thickness=2):
        self.lmList = []
        if self.results.pose_landmarks:
    ...
    def findAngle(self, img, p1, p2, p3, draw=True, color=(0,0,255), thickness=2):
        ...
        if draw:
            ...
            cv2.circle(img, (x1, y1), thickness, color, cv2.FILLED)
            cv2.circle(img, (x1, y1), thickness, color, cv2.FILLED)
            cv2.circle(img, (x1, y1), thickness, color, cv2.FILLED)
